...
function buildCurriculum(units, nodes, itemsByNode) {
  ...
  for (const unit of units) {
    ...
    const target = { coverageTargetId: targetId, blockId: unit.unitId, nodeId: node.nodeId, directSkillOrDecisionAtomId: unit.unitId, officialObjectiveRefs: unit.objectiveRefs, sourceRefs: [source.sourceId], learningObjective: unit.title, diagnosticDecision: PROFILES[unit.profileKey].rule, decisiveBoundary: PROFILES[unit.profileKey].boundary, transferBoundary: PROFILES[unit.profileKey].transfer, slotIds: items.map((item) => item.slotId), slotCount: items.length, sourceRequirements: { sourceRefs: [source.sourceId], resolutionState: "resolved_exact_direct" } };
    ...
    for (const item of items) {
      ...
      slots.push({ slotId: item.slotId, trackId: TRACK_ID, nodeId: node.nodeId, blockId: unit.unitId, coverageTargetId: targetId, officialObjectiveRefs: unit.objectiveRefs, directSkillOrDecisionAtomId: unit.unitId, learningOperation: scenario.family, questionIntent: item.prompt, materialEvidenceOrConstraintChanged: item.constraints, expectedOutcome: { kind: "choice", resolution: item.feedback.Reason }, errorModelOrFailureMode: scenario.targetedMisconception, decisiveBoundary: PROFILES[unit.profileKey].boundary, transferBoundary: PROFILES[unit.profileKey].transfer, intendedSurface: scenario.family, difficultyIntent: itemIndexDifficulty(item), eligibleModes: MODES, deliveryInteraction: { familyContract: "certification", interactionType: "choice", status: "existing_supported_but_not_runtime_admitted", selectionMode: "single" }, sourceRequirements: sourceReq, overlapExclusions: ["A semantically equivalent prompt with the same decisive constraint is a duplicate.", `Does not own adjacent units outside ${unit.unitId}.`], neighborDistinctness: [], dedupeFingerprint: sha256(canonical({ unitId: unit.unitId, caseId: scenario.caseId, constraint: scenario.constraint, family: scenario.family })), primarySimulationObjectiveRef: unit.objectiveRefs[0], primarySimulationDomainId: DOMAIN_IDS[node.domain], simulationOwnershipRationale: `Primary ownership follows the workbook mental unit ${unit.unitId}.` });
      objectiveAndDomainOwnership.push({ slotId: item.slotId, objectiveRef: unit.objectiveRefs[0], domainId: DOMAIN_IDS[node.domain], rationale: `Primary ownership follows the workbook mental unit ${unit.unitId}.` });
    }
  }
  const nodesOut = nodes.map((node, index) => { const nodeUnits = units.filter((unit) => unit.nodeId === node.nodeId); return { nodeId: node.nodeId, title: node.title, officialDomainId: DOMAIN_IDS[node.domain], freeOrPremiumRole: node.freeOrPremiumRole, prerequisiteNodeIds: index === 0 ? [] : [nodes[index - 1].nodeId], blockIds: nodeUnits.map((unit) => unit.unitId), slotCount: slots.filter((slot) => slot.nodeId === node.nodeId).length, learnerOutcome: `Master ${node.title.toLowerCase()} through source-grounded scenario decisions.` }; });
  const activeSourceIds = new Set(slots.flatMap((slot) => slot.sourceRequirements.directFirstPartyDocumentation.flatMap((requirement) => requirement.sourceRefs)));
  const curriculumSourceRecords = SOURCE_RECORDS.filter((source) => activeSourceIds.has(source.sourceId)).map((source) => ({ ...source, resolutionState: "resolved_exact_first_party", authoritativeFor: [...new Set(slots.filter((slot) => slot.sourceRequirements.directFirstPartyDocumentation.some((requirement) => requirement.sourceRefs.includes(source.sourceId))).map((slot) => slot.blockId))] }));
  const examSimulationBlueprint = { claim: "practice_simulation_not_provider_faithful", domainIds: Object.values(DOMAIN_IDS), sourceOfTruth: "current AI-901 study guide", officialAI901Profile: { guideVersion: "skills measured as of 2026-04-15", domainWeights: [{ domainId: DOMAIN_IDS.concepts, value: "50%", sourceRefs: ["microsoft-ai-901-study-guide"] }, { domainId: DOMAIN_IDS.implementation, value: "50%", sourceRefs: ["microsoft-ai-901-study-guide"] }] }, practiceForm: { itemCount: 50, domainAllocation: [{ domainId: DOMAIN_IDS.concepts, itemCount: 25 }, { domainId: DOMAIN_IDS.implementation, itemCount: 25 }], activeInteractionType: "choice" }, runtimeAdmission: "not_admitted" };
  const payload = { schemaVersion: "patternly-certification-curriculum-v1", curriculumVersion: CURRICULUM_VERSION, trackId: TRACK_ID, familyId: "certification", trackBriefReference: "docs/track-briefs/microsoft-azure-ai-fundamentals-ai-901.json", officialObjectiveRegistryRef: "config/certification-objective-registries/microsoft-azure-ai-fundamentals-ai-901.json", freeNodeId: nodes[0].nodeId, entryPrerequisites: ["Foundational Python syntax and programming techniques.", "Familiarity with Azure resources."], learnerOutcome: "Recognize and implement Azure AI solutions with Microsoft Foundry, including workload, capability, configuration, and responsible-use boundaries.", sourceRecords: curriculumSourceRecords, nodes: nodesOut, crossNodeRelationships: ["N01 owns conceptual Responsible AI, model, and deployment choices; N03 owns their implementation transfer.", "N02 owns workload classification; N04 and N05 own modality-specific implementation.", "N03 owns general Foundry client flow; N04 and N05 own modality-specific client flow."], blockPlans, targetPlans, slots, objectiveAndDomainOwnership, modeFeasibility: { freeNodeId: nodes[0].nodeId, premiumNodeIds: nodes.slice(1).map((node) => node.nodeId), allModes: MODES, interactionType: "choice", selectionMode: "single", minimumPool: 40 }, examSimulationBlueprint, targetItemCount: slots.length, existingVerifiedItemCount: 0, authoringItemCount: slots.length, admission: { learnerFacingContentIncluded: false, runtimeAdmission: "not_admitted", publishingAdmission: "not_admitted", questionsAuthored: 0 }, promotionProvenance: { sourceAuthority: "corrected AI-901 workbook and current Microsoft Study Guide", checkedDate: CREATED_AT, humanReviewRequired: true, approvalStatus: "unapproved" } };
...
async function main() {
  ...
  console.log(JSON.stringify({ trackId: TRACK_ID, nodes: nodes.map((node) => ({ nodeId: node.nodeId, questions: itemsByNode.get(node.nodeId).length })), mentalUnits: units.length, questions: units.reduce((sum, unit) => sum + unit.questionCount, 0), curriculumSlots: curriculum.slots.length }, null, 2));
}
