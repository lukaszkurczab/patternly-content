import { buildTrack, PublishingFailure, publishRelease, validateTrack, verifyArtifact } from "./pipeline.mjs";
const [command, ...args] = process.argv.slice(2); const value = (flag) => args[args.indexOf(flag) + 1];
try {
  if (command === "validate-track") console.log(JSON.stringify(await validateTrack({ trackId: value("--track") }), null, 2));
  ...
  else throw new PublishingFailure("USAGE", "Use validate-track, build-track, verify-artifact, or publish-release.");
