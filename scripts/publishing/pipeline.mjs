...
async function loadAlgorithmsFeedbackAssets(root) {
  const manifestPath = join(root, "manual", "assets", "algorithms", "feedback-assets.json");
  const schemaPath = join(root, "schemas", "publishing", "algorithms-feedback-assets.schema.json");
  const manifest = await json(manifestPath); const schema = await json(schemaPath);
  validateJsonSchema(manifest, schema, "Algorithms feedback asset manifest");
  if (manifest.schemaVersion !== "algorithms-feedback-assets-v1" || manifest.trackId !== "algorithms") throw new PublishingFailure("INVALID_ASSET_MANIFEST", "Algorithms feedback asset manifest identity is invalid.");
  const assets = list(manifest.assets, "Algorithms feedback assets", "INVALID_ASSET_MANIFEST").map((entry, index) => {
    const asset = record(entry, `Algorithms feedback asset ${index}`, "INVALID_ASSET_MANIFEST"); exactFields(asset, ["id", "sourcePath", "sha256"], `Algorithms feedback asset ${index}`, "INVALID_ASSET_MANIFEST");
    const id = text(asset.id, `Algorithms feedback asset ${index} id`, "INVALID_ASSET_MANIFEST"); const sourcePath = text(asset.sourcePath, `Algorithms feedback asset ${index} sourcePath`, "INVALID_ASSET_MANIFEST"); const sha256 = text(asset.sha256, `Algorithms feedback asset ${index} sha256`, "INVALID_ASSET_MANIFEST");
    if (!/^[a-z0-9][a-z0-9/_-]*$/.test(id) || !sourcePath.startsWith("manual/assets/algorithms/") || !sourcePath.endsWith(".svg") || !/^[a-f0-9]{64}$/.test(sha256)) throw new PublishingFailure("INVALID_ASSET_MANIFEST", `Algorithms feedback asset ${id} has an invalid local identity.`);
    return { id, sourcePath, sha256 };
  });
  unique(assets.map((asset) => asset.id), "DUPLICATE_ID", "Algorithms feedback asset IDs"); unique(assets.map((asset) => asset.sourcePath), "DUPLICATE_ID", "Algorithms feedback asset source paths");
  for (const asset of assets) {
    const path = inside(root, asset.sourcePath); let bytes;
    try { const info = await stat(path); if (!info.isFile()) throw new Error("not a file"); bytes = await readFile(path); } catch { throw new PublishingFailure("INVALID_ASSET_MANIFEST", `Algorithms feedback asset source is unavailable: ${asset.sourcePath}.`); }
    if (hash(bytes) !== asset.sha256) throw new PublishingFailure("INVALID_ASSET_MANIFEST", `Algorithms feedback asset hash differs from the manifest: ${asset.id}.`);
  }
...
function validateAlgorithmsItem(item, batchTaxonomy, taxonomy, provenance, feedbackAssets) {
...
function validateAlgorithmsSource(batches, track, family, taxonomyConfig, technicalInputFingerprint, sourceCommitValue, feedbackAssets) {
  const taxonomy = algorithmsTaxonomy(taxonomyConfig); const feedbackAssetsById = new Map(feedbackAssets.map((asset) => [asset.id, asset])); const first = batches[0]; const batchIds = [];
  ...
  for (const batch of batches) {
    ...
    contexts.push(...batch.items.map((item) => ({ batch, item: validateAlgorithmsItem(item, batchTaxonomy, taxonomy, provenance, feedbackAssetsById) })));
  }
...
export async function inspectTrack({ root = ROOT, trackId, sourceRepositoryCommit }) {
  ...
  const feedbackAssets = track.familyId === "algorithms" ? await loadAlgorithmsFeedbackAssets(root) : undefined;
  ...
  const source = track.familyId === "algorithms" ? validateAlgorithmsSource(batches, track, family, taxonomy, technicalInputFingerprint, commit, feedbackAssets) : validateCertificationSource(batches, track, family, taxonomy, technicalInputFingerprint, commit);
...
export async function buildReleaseCandidate({ root = ROOT, outputRoot, sourceRepositoryCommit }) {
  const tracks = ["algorithms", "cloud-certification"];
