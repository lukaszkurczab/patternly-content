...
async function main() {
  ...
  for (const node of nodes) {
    ...
    const contentBatchId = `fesd-${node.nodeId}-candidate-v1`;
    const batch = { schemaVersion: "frontend-system-design-interview-candidate-source-v1", candidateStatus: "generated_and_mechanically_validated_pending_human_review", activationState: "inactive_candidate", trackId: "frontend-system-design-interview", familyId: "system_design", runtimeAdmission: "not_admitted", publishingAdmission: "not_admitted", contentVersion: CONTENT_VERSION, taxonomyVersion: TAXONOMY_VERSION, nodeId: node.nodeId, nodeTitle: node.title, mentalUnitIds: nodeUnits.map((unit) => unit.unitId), competencyIds: node.competencyIds, items, authoringProvenance: { authoringMethod: "manual", approvalStatus: "unapproved", author: AUTHOR, createdAt: CREATED_AT, contentBatchId } };
    await writeJson(join(BANK_ROOT, `${node.nodeId}.content.json`), batch);
    nodeFiles.push({ nodeId: node.nodeId, path: `manual/source/frontend-system-design-interview/candidate-bank/${node.nodeId}.content.json`, questionCount: items.length, mentalUnitCount: nodeUnits.length });
  }
  const unitRecords = units.map((unit) => ({ ...unit, spec: unitSpecs.get(unit.unitId) }));
  const blueprint = { schemaVersion: "frontend-system-design-interview-candidate-blueprint-v1", ...workbookBaseline, trackId: "frontend-system-design-interview", family: "system_design", baseline: { ...workbookBaseline, exactGlobalTotal: allItems.length }, nodes: nodes.map((node) => ({ ...node, finalCount: nodeInventory(node, allItems).questionCount })), units: unitRecords, competencies, sourceRegistryPath: "manual/source/frontend-system-design-interview/candidate-bank/source-registry.json" };
  await writeJson(join(BANK_ROOT, "blueprint.json"), blueprint);
  await writeJson(join(BANK_ROOT, "source-registry.json"), { schemaVersion: "frontend-system-design-interview-source-registry-v1", trackId: "frontend-system-design-interview", family: "system_design", frameworkStatus: "source_grounded_synthesized_interview_framework_not_official_employer_syllabus", refreshedAt: "2026-08-15", sourceRecords: sourceRows, claimsAndAnchors: sourceClaims });
  await writeJson(join(BANK_ROOT, "coverage-matrix.json"), { schemaVersion: "frontend-system-design-interview-coverage-matrix-v1", trackId: "frontend-system-design-interview", generatedAt: CREATED_AT, units: unitRecords.map((unit) => ({ unitId: unit.unitId, nodeId: unit.nodeId, requiredDimensions: unit.spec.coverageDimensions, itemIds: unit.spec.itemIds, coverageGapAudit: unit.spec.gapAudit, saturationAudit: unit.spec.saturationAudit })) });
  await writeJson(join(BANK_ROOT, "item-intent-matrix.json"), { schemaVersion: "frontend-system-design-interview-item-intent-matrix-v1", trackId: "frontend-system-design-interview", generatedAt: CREATED_AT, itemCount: allItems.length, items: allItems.map((item) => ({ itemId: item.itemId, ...item.authoringIntent })) });
