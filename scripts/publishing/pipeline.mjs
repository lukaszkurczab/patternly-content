...
function humanEditorReviewer(value) {
  const reviewer = record(value, "approval reviewer", "INVALID_APPROVAL");
  if (reviewer.kind !== "human_editor" || Object.keys(reviewer).length !== 2) throw new PublishingFailure("INVALID_APPROVAL", "Approval reviewer must be a recorded human editor.");
  text(reviewer.id, "approval reviewer id", "INVALID_APPROVAL");
  return reviewer;
}
