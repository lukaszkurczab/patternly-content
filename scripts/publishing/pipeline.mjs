...
function certificationDiagnosticBaseline(track, items) {
  ...
  if (canonicalJson(Object.keys(configuration).sort(compare)) !== canonicalJson(["diagnosticBaseline", "focusPractice", "schemaVersion"]) || configuration.schemaVersion !== "certification-track-mode-config-v1") throw new PublishingFailure("INVALID_TRACK_MODE_CONFIGURATION", "Certification track mode configuration is invalid.");
...
function validateCertificationSource(batches, track, family, taxonomyConfig, technicalInputFingerprint, sourceCommitValue) {
  ...
  return { contentVersion: first.contentVersion, taxonomyVersion: first.taxonomyVersion, declaredModes, items: publishedItems, itemFingerprints, modeStructures: {}, examExperienceProfile: profile, diagnosticBaseline, focusPractice, batches, technicalEvidence, technicalInputFingerprint };
}
