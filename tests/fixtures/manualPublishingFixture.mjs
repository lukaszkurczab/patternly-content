...
export function algorithmsBatch({ count = 40, duplicate = false, badTaxonomy = false, invalidChoice = false, declaredModes = ["algorithms-interview-simulation"], poolIds, profile, modeStructures } = {}) {
  ...
  return {
    ...
    modeStructures: modeStructures ?? { practiceBlueprints: [{ blueprintId: "fixture-simulation-blueprint", blueprintVersion: "v1", modeId: "algorithms-interview-simulation", requestedLengths: [40], defaultRequestedLength: 40, shortening: "prohibited", minimumActualLength: 40, composition: { kind: "simulation_pool", ids: ["fixture-pool"] } }], recognitionSets: [], contrastSets: [], interleavedScopes: [], compatibilitySets: [], simulationPools: [{ poolId: "fixture-pool", poolVersion: "v1", itemIds: simulationPoolIds }], simulationProfiles: [simulationProfile] }
  };
}
...
export async function fixtureRoot(root, { algorithms, certification, approvals = true, legacy = false } = {}) {
  ...
  await write(root, "config/taxonomy/algorithms.json", { schemaVersion: "algorithms-taxonomy-v2", trackId: "algorithms", taxonomyVersion: "algorithms-taxonomy-v2", roadmapNodes: [{ id: "arrays_and_strings" }], mentalUnits: [{ id: "arrays_and_strings", roadmapNodeId: "arrays_and_strings" }], patternFamilies: [{ id: "arrays_and_strings", primaryMentalUnitId: "arrays_and_strings" }], patternVariants: [], problemArchetypes: [], skillAtoms: [{ id: "track_index_boundary", primaryMentalUnitId: "arrays_and_strings" }], falseHeuristics: [] });
