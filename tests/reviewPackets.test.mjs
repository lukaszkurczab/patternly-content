...
test("human review packets cover exactly the nine candidate tracks", async () => {
  ...
  for (const file of files) {
    ...
    assert.equal(packet.schemaVersion, "patternly-human-review-packet-v2");
    assert.match(packet.candidateId, /^[a-f0-9]{64}$/u);
    if (packet.trackId === "claude-certified-architect-professional-certification") {
      assert.equal(packet.approvalForm.status, "not_performed");
      assert.equal(packet.approvalForm.reviewer, null);
      assert.equal(packet.approvalForm.reviewScope.sampleCount, 0);
      assert.deepEqual(packet.sampleStrata.samples, []);
    } else {
      assert.equal(packet.approvalForm.status, "approved");
      assert.equal(packet.approvalForm.reviewer.kind, "human_owner");
      assert.equal(packet.approvalForm.reviewer.id, "lukaszkurczab");
      assert.equal(packet.approvalForm.reviewedAt, "2026-09-10");
    ...
    if (packet.trackId !== "claude-certified-architect-professional-certification") assert.ok(packet.sampleStrata.samples.length > 0);
