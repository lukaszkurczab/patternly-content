...
test("free-node inventory rejects a wrong node, a release-track mismatch, and a brief-track mismatch", async () => {
  const codingRelease = await release("patternly-launch-2026-08-25-01");
  ...
  assert.throws(() => inventoryFromPinnedRelease({ release: codingRelease, releaseId: "patternly-launch-2026-08-25-01", brief: missingNodeBrief, trackId: codingBrief.trackId, pin: codingPin }), fails("EMPTY_FREE_NODE"));
  assert.throws(() => inventoryFromPinnedRelease({ release: codingRelease, releaseId: "patternly-launch-2026-08-25-01", brief: codingBrief, trackId: "google-cloud-associate-cloud-engineer", pin: codingPin }), fails("BRIEF_TRACK_MISMATCH"));
});

test("free-node inventory refuses checksum-tampered pinned artifacts", async () => {
  const sourceRelease = await release("patternly-launch-2026-08-25-01");
  const codingBrief = await briefFor("coding-interview-dsa-problem-solving");
  const codingPin = await pinFor("coding-interview-dsa-problem-solving");
  ...
  assert.throws(() => inventoryFromPinnedRelease({ release: tamperedRelease, releaseId: "patternly-launch-2026-08-25-01", brief: codingBrief, trackId: "coding-interview-dsa-problem-solving", pin: codingPin }), fails("CHECKSUM_MISMATCH"));
});
...
test("free-node inventory allows only the exact canonical release pins", async () => {
  ...
  const backend = await generateFreeNodeInventory({ releaseId: "patternly-launch-2026-08-25-01", trackId: "backend-system-design-interview" });
...
test("free-node inventory validation detects fingerprint tampering and exact-set drift", async () => {
  ...
  const created = await writeFreeNodeInventory({ releaseId: "patternly-launch-2026-08-25-01", trackId: "coding-interview-dsa-problem-solving", outputPath: output });
