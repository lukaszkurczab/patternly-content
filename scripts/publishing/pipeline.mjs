...
export async function inspectTrack({ root = ROOT, trackId, sourceRepositoryCommit }) {
  ...
  const sourceSchema = await json(join(root, "schemas", "publishing", track.familyId === "coding_interview" ? "coding-interview-manual-source.schema.json" : "certification-manual-source.schema.json"));
  ...
  const source = track.familyId === "coding_interview" ? validateCodingInterviewSource(batches, track, family, taxonomy, technicalInputFingerprint, commit, feedbackAssets) : validateCertificationSource(batches, track, family, taxonomy, technicalInputFingerprint, commit);
