...
export async function auditCurricula() {
  ...
  const defects = {
    ...
    auditDimensions: [
      ...
      { dimension: "session_pool_and_modes", status: "certification_unverified", evidence: "GCP and Terraform completed exact-registry work but their mechanism documentation and later CERT-CORR-02 through CERT-CORR-04 remain blocked; the other four tracks remain registry_pending in CERT-CORR-01. Design remains blocked_by_contract." },
      ...
      { dimension: "source_and_provenance", status: "two_of_six_exact_registries_complete", evidence: "GCP and Terraform have verified exact registries and unresolved direct mechanism documentation; the remaining four exact registries remain pending their individual CERT-CORR-01 stages." },
      ...
      { dimension: "authoring_cost", status: "certification_volume_unverified", evidence: "Certification volume and its authoring gap are not accepted: GCP and Terraform await direct mechanism documentation and CERT-CORR-02 through CERT-CORR-04, while the other four tracks await their named CERT-CORR-01 stages." }
    ],
    defects: [
  ...
  const overlapAudit = { catalogueFingerprint: catalogue, status: "no_unowned_overlap_or_family_overfit", representativeContractChecks: [
    ...
    { familyId: "certification", representativeTracks: ["google-cloud-associate-cloud-engineer", "hashicorp-terraform-associate-004", "microsoft-azure-ai-fundamentals-ai-901"], controlTracks: ["aws-certified-solutions-architect-associate", "kubernetes-cloud-native-associate-kcna", "microsoft-azure-administrator-associate-az-104"], result: "relationship graph reconciled; GCP and Terraform exact objective ownership are complete, their later slot-plan work remains provisional, and the remaining four registries await their named CERT-CORR-01 stages" },
  ...
  const objectiveCoverage = { catalogueFingerprint: catalogue, certifications: curricula.filter((entry) => entry.familyId === "certification").map((entry) => {
    ...
    const unresolvedGuideGaps = entry.trackId === "google-cloud-associate-cloud-engineer" ? ["enterprise identity/setup and Cloud Asset Inventory/Gemini/WIF", "Agent Runtime, accelerators, notebooks and developer environments", "newer data/storage products, Database Center and CMEK", "AI-assisted tooling, networking, and operations mechanisms"] : ["direct first-party Terraform mechanism documentation for every target", "objective-level volume and slot distinctness validation in CERT-CORR-02 through CERT-CORR-04"];
