...
function validateCertificationSource(batches, track, family, taxonomyConfig, technicalInputFingerprint, sourceCommitValue) {
  ...
  return { contentVersion: first.contentVersion, taxonomyVersion: first.taxonomyVersion, declaredModes, items: publishedItems, itemFingerprints, modeStructures: {}, examExperienceProfile: profile, batches, technicalEvidence, technicalInputFingerprint };
}
...
function bankFor(validated) { if (validated.track.familyId !== "algorithms") return { formatVersion: 1, trackId: validated.track.trackId, familyId: validated.track.familyId, contentVersion: validated.source.contentVersion, examExperienceProfile: validated.source.examExperienceProfile, items: validated.source.items }; const modes = compileModeDeclarations(validated.source.modeStructures); return Object.freeze({ formatVersion: 1, trackId: "algorithms", familyId: "algorithms", contentVersion: validated.source.contentVersion, items: Object.freeze(validated.source.items.map((item) => compilePublishedAlgorithmsItem(item, item.compatibilityMemberships, item.itemFingerprint))), ...modes, approvalActivationIdentity: validated.approvalCoverage.identity }); }
