...
const SCORING_METHODS = Object.freeze({
  ...
  ordering: "adjacent_relations",
  complexity: "dimension_exact",
  decision_matrix: "dimension_exact"
});

const FEEDBACK_KINDS = Object.freeze({
  choice_single: Object.freeze(["wrong_option"]),
  choice_multiple: Object.freeze(["wrong_option", "omitted_option"]),
  ordering: Object.freeze(["wrong_element", "broken_relation"]),
  complexity: Object.freeze(["wrong_value", "omitted_dimension"]),
  decision_matrix: Object.freeze(["wrong_value", "omitted_dimension"])
});
...
function exactKeys(value, expectedKeys, pathName, errors, requiredKeys = expectedKeys) {
  if (!isRecord(value)) {
...
function validateJsonValue(value, pathName, errors) {
  ...
  if (Array.isArray(value)) {
    value.forEach((entry, index) => validateJsonValue(entry, `${pathName}[${index}]`, errors));
    return true;
  }
  if (isRecord(value)) {
    Object.entries(value).forEach(([key, entry]) => validateJsonValue(entry, `${pathName}.${key}`, errors));
    return true;
  }
  addError(errors, pathName, "must be a JSON value");
  return false;
}
...
function validateAliases(aliases, valueIds, pathName, errors) {
  if (!isRecord(aliases)) {
    addError(errors, pathName, "must be an object");
    return;
  }
...
function validateDimensions(dimensions, pathName, errors, type) {
  ...
  dimensions.forEach((dimension, index) => {
    ...
    if (type === "complexity" && Object.prototype.hasOwnProperty.call(dimension, "aliases")) validateAliases(dimension.aliases, valueIds, `${dimensionPath}.aliases`, errors);
    details.push({
      dimensionId: dimension.dimensionId,
      valueIds,
      acceptedValueIds: Array.isArray(accepted) ? accepted : [],
      aliases: type === "complexity" && isRecord(dimension.aliases) ? dimension.aliases : {}
    });
  });
...
function validateFeedback(feedback, type, referenceIds, dimensions, answer, pathName, errors) {
  ...
  if (!Array.isArray(feedback.messages)) {
    addError(errors, `${pathName}.messages`, "must be an array");
    return;
  }
  const allowedKinds = new Set(FEEDBACK_KINDS[type]);
  const targetSet = new Set(referenceIds);
  const dimensionIds = new Set(dimensions?.map((dimension) => dimension.dimensionId) ?? []);
  const validDimensionValueTargets = new Set(dimensions?.flatMap((dimension) => dimension.valueIds.map((valueId) => `${dimension.dimensionId}|${valueId}`)) ?? []);
  const validRelations = type === "ordering" && Array.isArray(answer?.orderedElementIds)
    ? answer.orderedElementIds.slice(0, -1).map((id, index) => `${id}->${answer.orderedElementIds[index + 1]}`)
    : [];
  for (const [index, message] of feedback.messages.entries()) {
    ...
    const validTarget = message.kind === "wrong_option" || message.kind === "omitted_option" || message.kind === "wrong_element"
      ...
      : message.kind === "broken_relation"
        ? validRelations.includes(message.targetId)
        : message.kind === "omitted_dimension"
          ? dimensionIds.has(message.targetId)
          : validDimensionValueTargets.has(message.targetId);
    if (!validTarget) addError(errors, `${messagePath}.targetId`, "does not reference this interaction");
  }
  const messageKeys = feedback.messages.map((message) => `${message?.kind}|${message?.targetId}`);
  if (new Set(messageKeys).size !== messageKeys.length) addError(errors, `${pathName}.messages`, "must not contain duplicate kind/targetId pairs");
}

function validateDimensionAnswer(answer, type, dimensions, pathName, errors) {
  if (!exactKeys(answer, ["type", "selectedValueIdsByDimension"], pathName, errors)) return;
  if (answer.type !== type) addError(errors, `${pathName}.type`, `must equal ${type}`);
  const selections = answer.selectedValueIdsByDimension;
  const dimensionIds = dimensions.map((dimension) => dimension.dimensionId);
  if (!isRecord(selections)) {
    addError(errors, `${pathName}.selectedValueIdsByDimension`, "must be an object");
    return;
  }
  exactIdSet(Object.keys(selections), dimensionIds, `${pathName}.selectedValueIdsByDimension`, errors);
  for (const dimension of dimensions) {
    const selectedPath = `${pathName}.selectedValueIdsByDimension.${dimension.dimensionId}`;
    const selected = selections[dimension.dimensionId];
    uniqueStrings(selected, selectedPath, errors, { minItems: 1 });
    if (Array.isArray(selected)) {
      for (const id of selected) if (!dimension.valueIds.includes(id)) addError(errors, selectedPath, `references an unknown value ${id}`);
      exactIdSet(selected, dimension.acceptedValueIds, selectedPath, errors);
    }
  }
}
...
function validateQuestionInternal(question, { catalogTrackIds } = {}) {
  ...
  if (type === "choice_single" || type === "choice_multiple") {
    if (!exactKeys(question.interaction, ["type", "scoringMethod", "options"], "question.interaction", errors)) return errors;
    if (question.interaction.scoringMethod !== SCORING_METHODS[type]) addError(errors, "question.interaction.scoringMethod", `must equal ${SCORING_METHODS[type]}`);
    ...
    if (type === "choice_single") {
      ...
      if (Array.isArray(ids)) for (const id of ids) if (!referenceIds.includes(id)) addError(errors, "question.answer.optionIds", `references unknown option ${id}`);
    }
  } else if (type === "ordering") {
    if (!exactKeys(question.interaction, ["type", "scoringMethod", "elements"], "question.interaction", errors)) return errors;
    if (question.interaction.scoringMethod !== SCORING_METHODS[type]) addError(errors, "question.interaction.scoringMethod", `must equal ${SCORING_METHODS[type]}`);
    referenceIds = validateElementList(question.interaction.elements, "question.interaction.elements", errors);
    if (exactKeys(question.answer, ["type", "orderedElementIds"], "question.answer", errors)) {
      if (question.answer.type !== type) addError(errors, "question.answer.type", `must equal ${type}`);
    ...
    if (!exactKeys(question.interaction, ["type", "scoringMethod", "dimensions"], "question.interaction", errors)) return errors;
    if (question.interaction.scoringMethod !== SCORING_METHODS[type]) addError(errors, "question.interaction.scoringMethod", `must equal ${SCORING_METHODS[type]}`);
    dimensions = validateDimensions(question.interaction.dimensions, "question.interaction.dimensions", errors, type);
    validateDimensionAnswer(question.answer, type, dimensions, "question.answer", errors);
  }

  validateFeedback(question.feedback, type, referenceIds, dimensions, question.answer, "question.feedback", errors);
  return errors;
}
...
function normalizeDimensionValue(dimension, valueId, type) {
  if (type === "complexity" && Object.prototype.hasOwnProperty.call(dimension.aliases ?? {}, valueId)) return dimension.aliases[valueId];
  return valueId;
}

function scoreDimensionResponse(question, response) {
  const type = question.interaction.type;
  ...
  for (const dimension of dimensions) {
    ...
    const selected = selections[dimension.dimensionId];
    const valueIds = dimension.values.map((value) => value.valueId);
    const aliases = type === "complexity" ? (dimension.aliases ?? {}) : {};
    if (!Array.isArray(selected) || new Set(selected).size !== selected.length || selected.some((id) => !valueIds.includes(id) && !Object.prototype.hasOwnProperty.call(aliases, id))) {
      return scoreResult(dimensions.length, 0, { invalidResponse: true, incorrectDimensionIds: dimensions.map((candidate) => candidate.dimensionId) });
    }
    const normalized = selected.map((id) => normalizeDimensionValue({ ...dimension, aliases }, id, type));
    if (new Set(normalized).size !== normalized.length) {
      return scoreResult(dimensions.length, 0, { invalidResponse: true, incorrectDimensionIds: dimensions.map((candidate) => candidate.dimensionId) });
    }
    const accepted = new Set(dimension.acceptedValueIds);
    if (normalized.length === accepted.size && normalized.every((id) => accepted.has(id))) correctDimensionIds.push(dimension.dimensionId);
    else incorrectDimensionIds.push(dimension.dimensionId);
  }
...
export function scoreQuestion(question, response) {
  ...
  if (type === "choice_single") {
    if (!responseShapeMatches(response, type, ["optionId"])) return scoreResult(1, 0, { invalidResponse: true });
    const selected = response?.optionId;
    const correct = selected === question.answer.optionId;
    return scoreResult(1, correct ? 1 : 0, { selectedOptionId: selected, correctOptionId: question.answer.optionId });
  }

  if (type === "choice_multiple") {
    if (!responseShapeMatches(response, type, ["optionIds"])) return scoreResult(question.interaction.options.length, 0, { invalidResponse: true });
    ...
    if (!Array.isArray(selected) || new Set(selected).size !== selected.length || selected.some((id) => !optionIds.includes(id))) {
  ...
  if (type === "ordering") {
    if (!responseShapeMatches(response, type, ["orderedElementIds"])) return scoreResult(question.answer.orderedElementIds.length - 1, 0, { invalidResponse: true });
  ...
  return scoreDimensionResponse(question, response);
}
