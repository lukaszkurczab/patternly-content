...
function sourceStatusForCertification(slot) {
  ...
  const valid = official?.registryRef && (official.objectiveRefs ?? []).length > 0 && (official.sourceRefs ?? []).length > 0 && docs.length > 0 && directSourceRefs.length > 0 && unresolved.length === 0 && docs.every((entry) => String(entry.resolutionState ?? "").startsWith("resolved"));
  const sourceRefs = sorted([...new Set([...(official?.sourceRefs ?? []), ...directSourceRefs])]);
  const claimIds = sorted(official?.objectiveRefs ?? []);
  const anchorIds = sorted([...new Set(docs.flatMap((entry) => entry.anchorPropertyRefs ?? entry.sourceRefs ?? []))]);
  return { admitted: !!valid, status: valid ? "exact_direct" : "blocked", reason: valid ? null : "missing_exact_direct_official_or_first_party_source_binding", binding: valid ? { bindingId: `${slot.trackId}:${slot.slotId}:source-binding`, claimIds, anchorIds, sourceRefs } : null };
}
...
export async function buildTrackPlan(root, track, model) {
  ...
  for (const block of normalized.blocks) {
    ...
    const sourcePaths = track.familyId === "coding_interview" ? (codingIndex.byBlock.get(block.blockId)?.sourcePaths ?? []) : slots.filter((slot) => slot.authoringAdmitted).map((slot) => slot.eventualSourcePath).filter(Boolean);
  ...
  return {
    ...
    learningBlocks: [...byBlock.values()].sort((a, b) => compare(a.learningBlockId, b.learningBlockId)),
    slots: slotPlans.sort((a, b) => compare(a.slotId, b.slotId)),
    wavePriority: [...byBlock.values()].filter((block) => block.authoringAdmittedItemCount > 0).sort((a, b) => compare(a.learningBlockId, b.learningBlockId)).map((block) => block.learningBlockId)
  };
}
...
export async function buildManifest(root = ROOT, options = {}) {
  ...
  const manifest = {
    ...
    tracks,
    gateResult: "READY_FOR_SCAFFOLDING_WITH_EXPLICIT_BLOCKS"
  };
