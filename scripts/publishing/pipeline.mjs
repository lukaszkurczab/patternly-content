...
const FEEDBACK_CODE_LANGUAGES = new Set(["pseudocode", "typescript", "python", "java", "cpp", "go", "sql"]);
const FEEDBACK_CALLOUT_KINDS = new Set(["decision_rule", "worked_example", "counterexample", "common_mistake", "edge_case", "complexity_note", "key_takeaway"]);
...
export const validateCanonicalJsonSchema = (value, schema, label = "document") => validateJsonSchema(value, schema, label);
...
async function loadCodingInterviewFeedbackAssets(root) {
  const manifestPath = join(root, "manual", "assets", "coding-interview-dsa-problem-solving", "feedback-assets.json");
  const schemaPath = join(root, "schemas", "publishing", "coding-interview-feedback-assets.schema.json");
  const manifest = await json(manifestPath); const schema = await json(schemaPath);
  validateJsonSchema(manifest, schema, "Coding Interview feedback asset manifest");
  if (manifest.schemaVersion !== "coding-interview-feedback-assets-v1" || manifest.trackId !== "coding-interview-dsa-problem-solving") throw new PublishingFailure("INVALID_ASSET_MANIFEST", "Coding Interview feedback asset manifest identity is invalid.");
  const assets = list(manifest.assets, "Coding Interview feedback assets", "INVALID_ASSET_MANIFEST").map((entry, index) => {
    const asset = record(entry, `Coding Interview feedback asset ${index}`, "INVALID_ASSET_MANIFEST"); exactFields(asset, ["id", "sourcePath", "sha256"], `Coding Interview feedback asset ${index}`, "INVALID_ASSET_MANIFEST");
    const id = text(asset.id, `Coding Interview feedback asset ${index} id`, "INVALID_ASSET_MANIFEST"); const sourcePath = text(asset.sourcePath, `Coding Interview feedback asset ${index} sourcePath`, "INVALID_ASSET_MANIFEST"); const sha256 = text(asset.sha256, `Coding Interview feedback asset ${index} sha256`, "INVALID_ASSET_MANIFEST");
    if (!/^[a-z0-9][a-z0-9/_-]*$/.test(id) || !sourcePath.startsWith("manual/assets/coding-interview-dsa-problem-solving/") || !sourcePath.endsWith(".svg") || !/^[a-f0-9]{64}$/.test(sha256)) throw new PublishingFailure("INVALID_ASSET_MANIFEST", `Coding Interview feedback asset ${id} has an invalid local identity.`);
    return { id, sourcePath, sha256 };
  });
  unique(assets.map((asset) => asset.id), "DUPLICATE_ID", "Coding Interview feedback asset IDs"); unique(assets.map((asset) => asset.sourcePath), "DUPLICATE_ID", "Coding Interview feedback asset source paths");
  for (const asset of assets) {
    const path = inside(root, asset.sourcePath); let bytes;
    try { const info = await stat(path); if (!info.isFile()) throw new Error("not a file"); bytes = await readFile(path); } catch { throw new PublishingFailure("INVALID_ASSET_MANIFEST", `Coding Interview feedback asset source is unavailable: ${asset.sourcePath}.`); }
    if (hash(bytes) !== asset.sha256) throw new PublishingFailure("INVALID_ASSET_MANIFEST", `Coding Interview feedback asset hash differs from the manifest: ${asset.id}.`);
  }
  return Object.freeze(assets.map((asset) => Object.freeze(asset)));
}
function exactFields(value, fields, label, code = "INVALID_RESPONSE") { if (Object.keys(value).length !== fields.length || Object.keys(value).some((key) => !fields.includes(key))) throw new PublishingFailure(code, `${label} has unsupported fields.`); }
function validateFeedbackDetails(value, label, feedbackAssets) {
  const document = record(value, label, "INVALID_RESPONSE"); exactFields(document, ["blocks"], label);
  const blocks = list(document.blocks, `${label}.blocks`, "INVALID_RESPONSE"); if (!blocks.length) throw new PublishingFailure("INVALID_RESPONSE", `${label}.blocks must not be empty.`);
  for (const [index, value] of blocks.entries()) {
    const block = record(value, `${label}.blocks[${index}]`, "INVALID_RESPONSE");
    const type = text(block.type, `${label}.blocks[${index}].type`, "INVALID_RESPONSE");
    if (type === "paragraph") { exactFields(block, ["type", "text"], `${label}.blocks[${index}]`); text(block.text, `${label}.blocks[${index}].text`, "INVALID_RESPONSE"); continue; }
    if (type === "heading") { exactFields(block, ["type", "level", "text"], `${label}.blocks[${index}]`); if (![2, 3].includes(block.level)) throw new PublishingFailure("INVALID_RESPONSE", `${label}.blocks[${index}].level is invalid.`); text(block.text, `${label}.blocks[${index}].text`, "INVALID_RESPONSE"); continue; }
    if (type === "bullet_list" || type === "ordered_list") { exactFields(block, ["type", "items"], `${label}.blocks[${index}]`); const items = list(block.items, `${label}.blocks[${index}].items`, "INVALID_RESPONSE"); if (!items.length) throw new PublishingFailure("INVALID_RESPONSE", `${label}.blocks[${index}].items must not be empty.`); items.forEach((item, itemIndex) => text(item, `${label}.blocks[${index}].items[${itemIndex}]`, "INVALID_RESPONSE")); continue; }
    if (type === "code") { exactFields(block, ["type", "language", "code"], `${label}.blocks[${index}]`); if (!FEEDBACK_CODE_LANGUAGES.has(block.language)) throw new PublishingFailure("INVALID_RESPONSE", `${label}.blocks[${index}].language is unsupported.`); text(block.code, `${label}.blocks[${index}].code`, "INVALID_RESPONSE"); continue; }
    if (type === "image") { exactFields(block, ["type", "assetId", "alt"], `${label}.blocks[${index}]`); const assetId = text(block.assetId, `${label}.blocks[${index}].assetId`, "INVALID_RESPONSE"); if (!/^[a-z0-9][a-z0-9/_-]*$/.test(assetId) || !feedbackAssets?.has(assetId)) throw new PublishingFailure("INVALID_RESPONSE", `${label}.blocks[${index}].assetId must reference a declared local Coding Interview feedback asset.`); text(block.alt, `${label}.blocks[${index}].alt`, "INVALID_RESPONSE"); continue; }
    if (type === "callout") { exactFields(block, block.title === undefined ? ["type", "kind", "text"] : ["type", "kind", "text", "title"], `${label}.blocks[${index}]`); if (!FEEDBACK_CALLOUT_KINDS.has(block.kind)) throw new PublishingFailure("INVALID_RESPONSE", `${label}.blocks[${index}].kind is unsupported.`); text(block.text, `${label}.blocks[${index}].text`, "INVALID_RESPONSE"); if (block.title !== undefined) text(block.title, `${label}.blocks[${index}].title`, "INVALID_RESPONSE"); continue; }
    throw new PublishingFailure("INVALID_RESPONSE", `${label}.blocks[${index}].type is unsupported.`);
  }
  return document;
}
function validateChoice(item, feedbackAssets) {
  ...
  const feedback = record(item.feedback, `${item.id} feedback`, "INVALID_RESPONSE"); text(feedback.reason, `${item.id} Reason`, "INVALID_RESPONSE"); validateFeedbackDetails(feedback.details, `${item.id} Details`, feedbackAssets);
...
function validateOrdering(item, feedbackAssets) {
  const interaction = record(item.interaction, `${item.id} interaction`, "INVALID_RESPONSE");
...
function validateComplexity(item, feedbackAssets) {
  const interaction = record(item.interaction, `${item.id} interaction`, "INVALID_RESPONSE");
...
function validateCodingInterviewItem(item, batchTaxonomy, taxonomy, provenance, feedbackAssets) {
  ...
  const response = item.interaction?.type === "choice" ? validateChoice(item, feedbackAssets) : item.interaction?.type === "ordering" ? validateOrdering(item, feedbackAssets) : item.interaction?.type === "complexity" ? validateComplexity(item, feedbackAssets) : (() => { throw new PublishingFailure("UNSUPPORTED_INTERACTION", `${id} interaction is unsupported.`); })();
...
const USER_MODE_MAPPINGS = [
  ...
  ["coding-interview-custom-practice", "coding-interview-guided-practice"],
  ["coding-interview-recognize-patterns", "coding-interview-recognize-patterns"],
...
function trackModeConfiguration(track) {
  ...
  if (configuration.schemaVersion !== "coding-interview-track-mode-config-v1") throw new PublishingFailure("INVALID_TRACK_MODE_CONFIGURATION", "Coding Interview track mode configuration schema is invalid.");
  const blueprints = list(configuration.practiceBlueprints, "Coding Interview track practiceBlueprints", "INVALID_TRACK_MODE_CONFIGURATION");
  if (blueprints.length !== PRACTICE_MODE_IDS.length) throw new PublishingFailure("INVALID_TRACK_MODE_CONFIGURATION", "Coding Interview track requires exactly six practice blueprints.");
  const modes = blueprints.map((blueprint) => text(record(blueprint, "Coding Interview track practice blueprint", "INVALID_TRACK_MODE_CONFIGURATION").modeId, "Coding Interview track practice blueprint modeId", "INVALID_TRACK_MODE_CONFIGURATION"));
  if (canonicalJson([...modes].sort(compare)) !== canonicalJson([...PRACTICE_MODE_IDS].sort(compare))) throw new PublishingFailure("INVALID_TRACK_MODE_CONFIGURATION", "Coding Interview track practice blueprints must own each canonical practice mode exactly once.");
  for (const blueprint of blueprints) {
    const value = record(blueprint, "Coding Interview track practice blueprint", "INVALID_TRACK_MODE_CONFIGURATION");
    for (const key of ["blueprintId", "blueprintVersion", "modeId", "selectionBoundary", "reinsertPolicy"]) text(value[key], `Coding Interview track practice blueprint ${key}`, "INVALID_TRACK_MODE_CONFIGURATION");
    const lengths = list(value.requestedLengths, "Coding Interview track practice blueprint requestedLengths", "INVALID_TRACK_MODE_CONFIGURATION"); if (!lengths.length || lengths.some((length) => !Number.isInteger(length))) throw new PublishingFailure("INVALID_TRACK_MODE_CONFIGURATION", "Coding Interview track practice blueprint lengths are invalid.");
    if (!lengths.includes(value.defaultRequestedLength) || !Number.isInteger(value.minimumActualLength) || value.minimumActualLength < 1 || !["allowed", "blueprint_controlled"].includes(value.shortening)) throw new PublishingFailure("INVALID_TRACK_MODE_CONFIGURATION", "Coding Interview track practice blueprint controls are invalid.");
    ids(value.learningStages, "Coding Interview track practice blueprint learningStages", "INVALID_TRACK_MODE_CONFIGURATION");
    if (value.modeId === "coding-interview-weak-area-review" && canonicalJson(ids(value.reviewSources, "Coding Interview weak-area review sources", "INVALID_TRACK_MODE_CONFIGURATION").sort(compare)) !== canonicalJson(["due_queue", "session_misses"])) throw new PublishingFailure("INVALID_TRACK_MODE_CONFIGURATION", "Weak Area Review sources must be due_queue and session_misses.");
  }
  const simulation = record(configuration.simulationBlueprint, "Coding Interview simulation blueprint", "MISSING_TRACK_MODE_CONFIGURATION");
  for (const key of ["blueprintId", "blueprintVersion", "modeId", "profileId", "profileVersion", "shorteningPolicy", "timerKind", "navigationPolicy", "answerChangePolicy", "reinsertPolicy", "feedbackTiming"]) text(simulation[key], `Coding Interview simulation blueprint ${key}`, "INVALID_TRACK_MODE_CONFIGURATION");
  if (simulation.modeId !== SIMULATION_MODE_ID || simulation.profileId !== "algorithms-interview-simulation-v1" || simulation.profileVersion !== "1" || simulation.requestedLength !== 40 || simulation.actualLength !== 40 || simulation.shorteningPolicy !== "prohibited" || simulation.uniqueItemsRequired !== 40 || simulation.timerKind !== "foreground_countdown" || simulation.durationMinutes !== 45 || simulation.navigationPolicy !== "free_navigation" || simulation.answerChangePolicy !== "editable_until_finalization" || simulation.reinsertPolicy !== "disabled" || simulation.feedbackTiming !== "after_verified_finalization") throw new PublishingFailure("INVALID_TRACK_MODE_CONFIGURATION", "Coding Interview simulation blueprint conflicts with the approved fixed-40 contract.");
  const policy = record(simulation.selectionPolicy, "Coding Interview simulation selection policy", "INVALID_TRACK_MODE_CONFIGURATION");
  for (const key of ["requireUniqueItemIds", "requireDeclaredSimulationEligibility", "requireMultipleMentalUnits", "requireMultiplePatternFamilies", "requireEveryActiveInteractionTypeRepresented", "prohibitConsecutiveSameMentalUnitWhenAlternativeExists", "prohibitDuplicateContentIdentity", "prohibitTaxonomyWidening", "prohibitFallbackItems"]) if (policy[key] !== true) throw new PublishingFailure("INVALID_TRACK_MODE_CONFIGURATION", "Coding Interview simulation selection policy is incomplete.");
  const userModeMappings = list(configuration.userModeMappings, "Coding Interview userModeMappings", "INVALID_TRACK_MODE_CONFIGURATION").map((mapping) => {
    const value = record(mapping, "Coding Interview user mode mapping", "INVALID_TRACK_MODE_CONFIGURATION");
    return [text(value.userModeId, "Coding Interview user mode ID", "INVALID_TRACK_MODE_CONFIGURATION"), text(value.blueprintModeId, "Coding Interview user mode blueprint ID", "INVALID_TRACK_MODE_CONFIGURATION")];
  });
  if (canonicalJson(userModeMappings) !== canonicalJson(USER_MODE_MAPPINGS)) throw new PublishingFailure("INVALID_TRACK_MODE_CONFIGURATION", "Coding Interview user mode mappings must cover exactly the canonical eight modes.");
...
function validateModeStructures(structures, declaredModes, family, track, items, taxonomy) {
  ...
  for (const modeId of declaredModes) {
    ...
    const requirement = requirementByMode.get(modeId); if (!requirement) throw new PublishingFailure("INVALID_MODE", `Unknown Coding Interview mode ${modeId}.`);
...
function compileFeedbackDetails(details) { return Object.freeze({ blocks: Object.freeze(details.blocks.map((block) => Object.freeze(block.type === "bullet_list" || block.type === "ordered_list" ? { ...block, items: Object.freeze([...block.items]) } : { ...block }))) }); }
export function compileChoiceItem(item, compatibilityMemberships, itemFingerprint) { return Object.freeze({ id: item.id, prompt: item.prompt, ...(item.constraints ? { constraints: item.constraints } : {}), ...(item.difficulty ? { difficulty: item.difficulty } : {}), interaction: Object.freeze({ type: "choice", selectionMode: item.interaction.selectionMode, options: Object.freeze(item.interaction.options.map((option) => Object.freeze({ id: option.id, text: option.text }))), acceptedOptionIds: Object.freeze([...item.interaction.acceptedOptionIds]) }), scoringContract: Object.freeze({ type: "choice", resultSemantics: "exact_selected_set_with_partial_v1" }), feedback: Object.freeze({ reason: item.feedback.reason, details: compileFeedbackDetails(item.feedback.details), ...(item.feedback.wrongOptionExplanationsByOptionId ? { wrongOptionExplanationsByOptionId: Object.freeze({ ...item.feedback.wrongOptionExplanationsByOptionId }) } : {}), ...(item.feedback.omittedCorrectExplanationsByOptionId ? { omittedCorrectExplanationsByOptionId: Object.freeze({ ...item.feedback.omittedCorrectExplanationsByOptionId }) } : {}) }), taxonomy: compileResolvedTaxonomy(item), provenance: compileResolvedProvenance(item), compatibilityMemberships: Object.freeze([...compatibilityMemberships]), itemFingerprint }); }
export function compileOrderingItem(item, compatibilityMemberships, itemFingerprint) { return Object.freeze({ id: item.id, prompt: item.prompt, ...(item.constraints ? { constraints: item.constraints } : {}), ...(item.difficulty ? { difficulty: item.difficulty } : {}), interaction: Object.freeze({ type: "ordering", elements: Object.freeze(item.interaction.elements.map((element) => Object.freeze({ id: element.id, text: element.text }))), canonicalOrder: Object.freeze([...item.interaction.canonicalOrder]), scoringMethod: "adjacent_relations" }), scoringContract: Object.freeze({ type: "ordering", maxPoints: item.scoringContract.maxPoints }), feedback: Object.freeze({ reason: item.feedback.reason, details: compileFeedbackDetails(item.feedback.details) }), taxonomy: compileResolvedTaxonomy(item), provenance: compileResolvedProvenance(item), compatibilityMemberships: Object.freeze([...compatibilityMemberships]), itemFingerprint }); }
export function compileComplexityItem(item, compatibilityMemberships, itemFingerprint) { return Object.freeze({ id: item.id, prompt: item.prompt, ...(item.constraints ? { constraints: item.constraints } : {}), ...(item.difficulty ? { difficulty: item.difficulty } : {}), interaction: Object.freeze({ type: "complexity", checkedDimensions: Object.freeze([...item.interaction.checkedDimensions]), availableValuesByDimension: Object.freeze({ ...item.interaction.availableValuesByDimension }), acceptedValuesByDimension: Object.freeze({ ...item.interaction.acceptedValuesByDimension }), normalizedAliasesByDimension: Object.freeze({ ...item.interaction.normalizedAliasesByDimension }), ...(item.interaction.sharedPresetId ? { sharedPresetId: item.interaction.sharedPresetId } : {}), maxPoints: item.interaction.maxPoints }), scoringContract: Object.freeze({ type: "complexity", maxPoints: item.scoringContract.maxPoints }), feedback: Object.freeze({ reason: item.feedback.reason, details: compileFeedbackDetails(item.feedback.details) }), taxonomy: compileResolvedTaxonomy(item), provenance: compileResolvedProvenance(item), compatibilityMemberships: Object.freeze([...compatibilityMemberships]), itemFingerprint }); }
export function compilePublishedCodingInterviewItem(item, compatibilityMemberships, itemFingerprint) { if (item.interactionType === "choice") return compileChoiceItem(item, compatibilityMemberships, itemFingerprint); if (item.interactionType === "ordering") return compileOrderingItem(item, compatibilityMemberships, itemFingerprint); return compileComplexityItem(item, compatibilityMemberships, itemFingerprint); }
...
function validateCodingInterviewSource(batches, track, family, taxonomyConfig, technicalInputFingerprint, sourceCommitValue, feedbackAssets) {
  const taxonomy = codingInterviewTaxonomy(taxonomyConfig); const feedbackAssetsById = new Map(feedbackAssets.map((asset) => [asset.id, asset])); const first = batches[0]; const batchIds = [];
  for (const batch of batches) {
    if (batch.schemaVersion !== "coding-interview-manual-source-v2" || batch.trackId !== track.trackId || batch.familyId !== "coding_interview" || batch.taxonomyVersion !== track.taxonomyVersion) throw new PublishingFailure("INVALID_ENVELOPE", "Coding Interview batch envelope conflicts with track configuration.");
    batchIds.push(text(batch.batchId, "batchId")); text(batch.contentVersion, "contentVersion"); if (batch.contentVersion !== first.contentVersion || canonicalJson(batch.declaredModes) !== canonicalJson(first.declaredModes)) throw new PublishingFailure("VERSION_MISMATCH", "Coding Interview batches must share one content version and mode declaration.");
  }
...
function validateCertificationExamExperienceProfile(value) {
  ...
  const sectionIds = sections.map((section) => { const item = record(section, "Certification exam experience profile section", "INVALID_EXAM_EXPERIENCE_PROFILE"); if (canonicalJson(Object.keys(item).sort(compare)) !== canonicalJson(["contentDomainId", "id", "weightPercent"])) throw new PublishingFailure("INVALID_EXAM_EXPERIENCE_PROFILE", "Certification exam experience profile section has unsupported fields."); text(item.contentDomainId, "Certification exam experience profile section content domain", "INVALID_EXAM_EXPERIENCE_PROFILE"); return text(item.id, "Certification exam experience profile section ID", "INVALID_EXAM_EXPERIENCE_PROFILE"); });
...
function certificationMixedPractice(track, items) {
  ...
  if (itemIds.length < 10 || new Set(itemIds).size !== itemIds.length || itemIds.some((itemId) => !items.some((item) => item.id === itemId))) throw new PublishingFailure("INVALID_TRACK_MODE_CONFIGURATION", "Certification Mixed Practice must name unique current interleaved items.");
  return Object.freeze({ ...mixed, requestedLengths: Object.freeze([...requestedLengths]), itemIds: Object.freeze([...itemIds]) });
}
...
function certificationModeReadiness({ items, declaredModes, profile, diagnosticBaseline, focusPractice, scenarioPractice, weakAreaReview, mixedPractice, quickReview }) {
  ...
  const readiness = [
    ...(declaredModes.includes(diagnosticBaseline.modeId) ? [scoped(diagnosticBaseline.modeId, { kind: "explicit_item_ids", id: diagnosticBaseline.blueprintId }, [diagnosticBaseline.requestedLength], diagnosticBaseline.shortening, diagnosticBaseline.itemIds.length, diagnosticBaseline.uniqueItemsRequired)] : []),
    ...(declaredModes.includes(focusPractice.modeId) ? focusPractice.topicIds.map((topicId) => scoped(focusPractice.modeId, { kind: focusPractice.selectionScope, id: topicId }, focusPractice.requestedLengths, focusPractice.shortening, items.filter((item) => item.domain === topicId).length, 10)) : []),
    ...(declaredModes.includes(scenarioPractice.modeId) ? scenarioPractice.competencies.map((competency) => scoped(scenarioPractice.modeId, { kind: scenarioPractice.selectionScope, id: competency.id }, scenarioPractice.requestedLengths, scenarioPractice.shortening, competency.scenarioItemIds.length, 10)) : []),
    ...(declaredModes.includes(weakAreaReview.modeId) ? [scoped(weakAreaReview.modeId, { kind: weakAreaReview.selectionScope, id: "catalog" }, weakAreaReview.requestedLengths, weakAreaReview.shortening, items.length, 1)] : []),
    ...(declaredModes.includes(mixedPractice.modeId) ? [scoped(mixedPractice.modeId, { kind: mixedPractice.selectionScope, id: mixedPractice.blueprintId }, mixedPractice.requestedLengths, mixedPractice.shortening, mixedPractice.itemIds.length, 10)] : []),
    ...(declaredModes.includes(quickReview.modeId) ? [scoped(quickReview.modeId, { kind: quickReview.selectionScope, id: "catalog" }, [quickReview.maximumLength], quickReview.shortening, items.length, 1)] : []),
...
function validateCertificationSource(batches, track, family, taxonomyConfig, technicalInputFingerprint, sourceCommitValue) {
  ...
  for (const batch of batches) {
    if (batch.trackId !== track.trackId || batch.familyId !== track.familyId || batch.contentVersion !== first.contentVersion || batch.taxonomyVersion !== track.taxonomyVersion) throw new PublishingFailure("INVALID_ENVELOPE", "Certification batch envelope conflicts with track configuration.");
    batchIds.push(text(batch.batchId, "Certification batchId"));
  }
  unique(batchIds, "DUPLICATE_ID", "Certification batch IDs"); const declaredModes = ids(sourceVersion === "certification-manual-source-v1" ? [...new Set(batches.flatMap((batch) => batch.declaredModes ?? []))].sort(compare) : [...new Set(batches.flatMap((batch) => batch.items.flatMap((item) => item.modeEligibility ?? [])))].sort(compare), "Certification declaredModes", "INVALID_MODE");
  if (sourceVersion === "certification-manual-source-v1" ? canonicalJson([...declaredModes].sort(compare)) !== canonicalJson([...legalModes].sort(compare)) : declaredModes.some((modeId) => !legalModes.includes(modeId))) throw new PublishingFailure("INVALID_MODE", "Certification declared modes must be a legal family mode set.");
...
const DURABLE_EVIDENCE_GENERATOR_VERSION = "coding-interview-release-evidence-v1";
...
function simulationCoverage(inspected) {
  ...
  if (pools.length !== 1 || profiles.length !== 1) throw new PublishingFailure("INVALID_SIMULATION_EVIDENCE", "Coding Interview release evidence requires exactly one simulation pool and profile.");
  ...
  return {
    ...
    roadmapNodeCoverage: coverage((item) => item.resolvedTaxonomy.roadmapNodeId), mentalUnitCoverage: coverage((item) => item.resolvedTaxonomy.primaryMentalUnitId), patternFamilyCoverage: coverage((item) => item.resolvedTaxonomy.patternFamilyId), patternVariantCoverage: coverage((item) => item.resolvedTaxonomy.patternVariantId), problemArchetypeCoverage: coverage((item) => item.resolvedTaxonomy.problemArchetypeId), primarySkillAtomCoverage: coverage((item) => item.resolvedTaxonomy.primarySkillAtomId), interactionTypeCoverage: coverage((item) => item.interactionType), learningStageCoverage: coverage((item) => item.resolvedTaxonomy.learningStage), coveragePolicyVersion: "coding-interview-simulation-coverage-v1"
  };
}
